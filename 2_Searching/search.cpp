#include <iostream>
using namespace std;

class MyArray{
    private:
        int total_size;
        int used_size;
        int *ptr;

    public:
        //constructer
        MyArray(int tsize, int usize){
            total_size = tsize;
            used_size = usize;
            ptr = new int[tsize];
        }

        //getters
        int get_total_size(){
            return total_size;
        }

        int get_used_size(){
            return used_size;
        }

        int* get_ptr(){
            return ptr;
        }
   
        //takes input for values of array
        void set_array(){
            int i, inp;

            cout<<"Enter values for array"<<endl;
            for(i=0; i<used_size; i++){
                cin >> inp;
                ptr[i] = inp;
            }
        }

        void delet_array(){
            delete ptr;
        }

        //print array
        void traversal(){
            int i;

            cout<<"Current values in array"<<endl;
            for(i=0;i<used_size;i++){
                cout << ptr[i] <<" ";
            }

            cout << endl;
        }

        //linear search
        int linear_search(int element){
            int i;

            for(i=0; i <used_size ; i++){
                if(ptr[i] == element){
                    return i;
                }
            }

            return -1;
        }

        //binary search(for sorted arrays only)
        int binary_search(int element, int low, int high){
            int mid;

            if(low <= high){
                mid = (low + high)/2;

                if(ptr[mid] == element){
                    return mid;
                }
                else if(element < ptr[mid]){
                    high = mid - 1;
                    return binary_search(element, low, high);
                }
                else if(element > ptr[mid]){
                    low =  mid + 1;
                    return binary_search(element, low, high);
                }
            }

            return -1;
        }
};

int main(){
    int max_size, size;
    int element, status;
    int low, high;

    cout << "Enter max size you want for array --> ";
    cin >> max_size;

    cout << "Enter size of array you want to use --> ";
    cin >> size;

    MyArray arr(max_size, size);
    MyArray* arr_ptr = &arr;

    //setting array
    cout << endl;
    (*arr_ptr).set_array();

    cout << endl;
    (*arr_ptr).traversal();

    cout<<endl;
    cout << "Enter element to be searched --> ";
    cin >> element;

    cout << endl;
    cout <<"LINEAR SEARCH"<<endl;

    //using linear search
    status = (*arr_ptr).linear_search(element);
    if(status == -1){
        cout << "Element doesn't exist in array" << endl;
    }
    else{
        cout << "Element found at index --> " << status <<endl;
    }

    cout << endl;
    cout <<"BINARY SEARCH"<<endl;

    //using binary search
    low = 0;
    high = (*arr_ptr).get_used_size() - 1;

    status = (*arr_ptr).binary_search(element, low, high);
    if(status == -1){
        cout << "Element doesn't exist in array" << endl;
    }
    else{
        cout << "Element found at index --> " << status <<endl;
    }

    (*arr_ptr).delet_array();

    return 0;
}